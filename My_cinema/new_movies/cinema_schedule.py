...
def generate_july_week_schedule(schedule):
    july = date(year=2022, month=7, day=11)

    result = {}
    for weekday, showtimes in schedule.items():
        particular_weekday_in_july = july.replace(day=july.day + weekday.value)
        result[weekday] = [
            MovieShowDatetime.from_movie_showtime_and_date(
                movie_showtime, particular_weekday_in_july
            )
